...
#Reading in the query, account for input both formatted as string and otherwise
    ...
    #Finding the query:
    if type(query) == str:
        sqlCommand = """SELECT idNumber FROM taxInfo_names WHERE upper(altName) = '{}';""".format(query)
        crsr.execute(sqlCommand)
        try:
            idNumber = crsr.fetchone()[0]
        except TypeError:
    ...
    if int(idNumber) == 0:
        ...
        sqlCommand = """SELECT idNumber, parent, sciName, comName, genComName, gcID, mgcID FROM taxInfo_tax WHERE idNumber = {};""".format(idNumber)
        crsr.execute(sqlCommand)
        ...
        while parent != 1 and parent !=0:  #As 1 is the root containing all organisms, all paths should lead back to 1. In case 1 is used as query, parent of 0 is also excluded.
            sqlCommand = """SELECT parent, sciName FROM taxInfo_tax WHERE idNumber = {};""".format(str(parent))
            crsr.execute(sqlCommand)
            speciesInfo = crsr.fetchone()
        ...
        #Printing output
        print("Taxon ID:\t\t\t{}".format(idNumber))
